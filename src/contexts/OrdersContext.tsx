...
interface OrdersContextType {
  orders: Order[];
  addOrder: (order: Omit<Order, 'id' | 'date'>) => Promise<Order | null>;
...
export const OrdersProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
  ...
  const addOrder = async (orderData: Omit<Order, 'id' | 'date'>): Promise<Order | null> => {
  ...
  const updateOrderStatus = async (id: string, status: Order['status']) => {
    ...
    // Retrieve order details to send the status update email
    const existingOrder = orders.find(o => o.id === id);
    if (existingOrder) {
      const orderForEmail = { ...existingOrder, status };
